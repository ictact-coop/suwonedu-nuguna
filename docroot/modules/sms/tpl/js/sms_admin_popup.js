...
var addrview = {
	groupNavi: function(page) {
		...
		var f = document.smsMemberForm;
		//////// 그룹에 속한 회원정보를 가져온다 ////////
		procFilter(f,select_group_member);
	...
	memberDel: function(idx,uid) {
		...
		if(confirm(lang+lang_sms_donot_send+lang_sms_msg)) {
			...
			f.uid.value = idx;
			f.bsort.value   = 'user';
			procFilter(f,delete_group_member);
		}
	},
	memberInsert: function(idx,uid) {
		...
		f.bsort.value = 'user';
		f.uid.value =   idx;
		procFilter(f,insert_sms_address);
	},
	memberAll: function() {
		var f = document.smsMemberForm;
		...
		if(confirm(lang+lang_sms_do_send+lang_sms_msg)) {
			...
			procFilter(f,insert_all_address);
		}
	},
	memberAllOut: function() {
		...
		if(confirm(lang+lang_sms_donot_send+lang_sms_msg)) {
			...
			f.bsort.value = 'alluser';
			procFilter(f,delete_all_address);
		}
	},
	...
	memberUse: function(userId,check,gid) {
		...
		if(confirm(lang+msg+lang_sms_msg)) {
			...
			f.mode.value = check;
			//////// 그룹에 속한 회원정보를 가져온다 ////////
			procFilter(f,update_group_member);
		}
	},
...
function completeDelMember(ret_obj) {
	...
	if(data.code == 'fail') {
		...
		top.window.popWindow.memberDel(data.delMember);
...
function completeSmsAddress(ret_obj) {
	...
	if(data.code == 'yes' || data.code == 'success' || data.code == 'f227') {
		...
		$(uid+'_span').innerHTML = tag;
		top.window.popWindow.memberAdd(data.addMember);
...
function completeSmsAllAddress(ret_obj) {
	...
	if(data.code == 'fail') {
		...
		top.window.popWindow.memberDel(data.delMember,'allOut');
		location.reload();
	} else {
		var f = document.smsMemberForm;
		location.href = './?module=sms&act=dispSmsAdminMemberAddrSetting&module_srl='+f.bcode.value+'&bcate='+f.bcate.value+'&admin_send=direct';
		top.window.popWindow.memberAdd(data.addMember,'allIn');
	}
...
function checkMember(msrl) {
	...
	location.href='./?module=sms&act=dispSmsAdminMemberAddrSetting&module_srl='+msrl+'&addr_use='+addruse;
}
