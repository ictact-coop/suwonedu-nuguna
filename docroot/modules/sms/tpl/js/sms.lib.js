...
var per_sms = {
	...
	getByteLength : function(target) {
		...
		var checkPattern = /[%&]/;
		var pattern = /[a-z,A-Z,1-9,0!@#$^*()=\/?\-_+|~\"\'`\]\[.\,<>:\;\{\} ]/;
		var lengthText = 0;

		if(checkPattern.test(content)) return false;

		for(var i = 0; i < content.length; i++) {
			if(pattern.test(content.substring(i,parseInt(i)+1))) {
				lengthText = parseInt(lengthText) +1;
			} else lengthText = parseInt(lengthText) +2;
		}

		if(lengthText > limit_byte) {
			sms_count = Math.ceil(lengthText / limit_byte);
			limit_byte = limit_byte * sms_count;
		}

		return {'length':lengthText, 'sms_count':sms_count, 'limit_byte':limit_byte};
	},
	showAddressbook : function(mode) {
		if(mode == 'window') {
			console.log('window');
		}
	}
...
$.extend(window.PerbizSMS, per_sms);
