<?php
    ...
    class xe_popular extends WidgetHandler {
        ...
        function proc($args) {
            ...
            // 최신글을 구함
            $output = executeQueryArray('widgets.xe_popular.getNewestDocuments', $db_args);
            if($output->data) {
            ...
            $output = executeQueryArray('widgets.xe_popular.getNewestComments', $db_args);
            if($output->data) {
                foreach($output->data as $k => $v) {
                    $oComment = null;
            ...
            $output = executeQueryArray('widgets.xe_popular.getPopularDocuments', $db_args);
            if($output->data) {
                foreach($output->data as $k => $v) {
                    $oDocument = null;
                    $oDocument = $oDocumentModel->getDocument();
