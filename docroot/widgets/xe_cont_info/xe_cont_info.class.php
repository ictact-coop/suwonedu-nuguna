 ...
 * @version 0.1
 */
class xe_cont_info extends WidgetHandler
{
	/**
	...
	function proc($args)
	{
		...
		else
		{
			...
			if(!$args->module_srls)
			{
				...
				$output = executeQueryArray('widgets.xe_cont_info.getMids', $obj);
				if($output->data)
				{
	...
	function _getDocumentItems($args)
	{
		...
		$obj = new stdClass();
		$obj->module_srl = $args->module_srl;
		$output = executeQueryArray('widgets.xe_cont_info.getCategories',$obj);
		...
		$output = executeQueryArray('widgets.xe_cont_info.getNewestDocuments', $obj);
