<?php
    ...
    class xe_content extends WidgetHandler {
        ...
        function proc($args) {
            ...
            if(!$args->thumbnail_width) $args->thumbnail_width = 400;
            // Vertical size of thumbnails
            if(!$args->thumbnail_height) $args->thumbnail_height = 200;
            // Viewing options
            ...
            // Get module information after listing module_srls if the module is not RSS
                ...
                if(!$args->module_srls){
                    ...
                    $output = executeQueryArray('widgets.xe_content.getMids', $obj);
                    if($output->data) {
        ...
        /**
         * @brief Get a list of comments and return xe_contentItem
         **/
        function _getCommentItems($args) {
            ...
            foreach($output as $key => $oComment) {
                ...
                $content_item = new xe_contentItem($browser_title);
        ...
        function _getDocumentItems($args){
            ...
            // Get categories
            $obj->module_srl = $args->module_srl;
            $output = executeQueryArray('widgets.xe_content.getCategories',$obj);
            ...
            $output = executeQueryArray('widgets.xe_content.getNewestDocuments', $obj);
            ...
            if(count($output->data)) {
                ...
                for($i=0,$c=count($document_srls);$i<$c;$i++) {
                    ...
                    $content_item = new xe_contentItem( $args->module_srls_info[$module_srl]->browser_title );
       ...
       /**
        ...
        function _getRssItems($args){
            ...
            if($xml_doc->rss) {
                ...
                foreach ($items as $key => $value) {
                    ...
                    $content_item = new xe_contentItem($rss->title);
        ...
        function _compile($args,$content_items){
			...
			$widget_info->content_title = $args->content_title;
            $widget_info->content_title_url = $args->content_title_url;
    ...
    class xe_contentItem extends Object {
        ...
        function xe_contentItem($browser_title=''){
		...
		function getProfileImage() {
            ...
            $pi = $oMemberModel->getProfileImage($member_srl);
            return $pi->src;
        }
		function getAfterRegdate() {
            $regdate = $this->get('regdate');
            $gap = time() + zgap() - ztime($regdate) ;
            ...
            elseif($gap<60*60*24*2) $buff =  '어제';
			elseif($gap<60*60*24*7) $buff =   (int)($gap / 60 /60/24)+1 .'일전';
            else $buff =  $this->getRegdate("m월 d일");
            return $buff;
